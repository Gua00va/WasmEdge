...
public:
  ...
  ValMut getMutability() const noexcept { return Mut; }
...
public:
  /// Constructors.
  CompositeType() noexcept = delete;
  CompositeType(const FieldType &FT) noexcept {
    FType = std::vector<FieldType>{FT};
  }
  CompositeType(const std::vector<FieldType> &VFT) noexcept : FType(VFT) {}
  CompositeType(const FunctionType &FT) noexcept : FType(FT) {}
  ...
  /// Getter of checking content type.
  bool isFuncType() const noexcept {
    return std::holds_alternative<FunctionType>(FType);
  }
  bool isFieldTypes() const noexcept {
    return std::holds_alternative<std::vector<FieldType>>(FType);
  }
...
public:
  /// Constructors.
  SubType(bool IsFinal, const std::vector<uint32_t> &TIdx,
          const CompositeType &CT) noexcept
      : IsFinal(IsFinal), TypeIndex(TIdx), CompType(CT) {}
  SubType(const CompositeType &CT) noexcept : IsFinal(true), CompType(CT) {}

  /// Getter of checking is final.
  bool isFinal() const noexcept { return IsFinal; }

  /// Getter of type index vector.
  const std::vector<uint32_t> &getTypeIndices() const noexcept {
    return TypeIndex;
  }
...
/// AST RecursiveType node for GC proposal.
class RecursiveType {
public:
  /// Constructors.
  RecursiveType() noexcept = default;
  RecursiveType(const FunctionType &FT) noexcept {
    STypes.emplace_back(CompositeType(FT));
  }

  /// Getter of sub types.
  const std::vector<SubType> &getSubTypes() const noexcept { return STypes; }
  std::vector<SubType> &getSubTypes() noexcept { return STypes; }
